...
## Кнопка вимкнення оверлея на відео (позапланово)

Користувач попросив можливість вимикати індикатори прямо на відео (окремо від уже наявного
збоку булевого перемикача в `LandmarkIndicator`, який показує/ховає крапки "рука видна: так/ні"
поза кадром, а не сам скелет на відео).

- **`frontend/components/Camera/Camera.tsx`** — новий локальний стан `overlayEnabled` (типово
  `true`, бо це основний спосіб побачити, що розпізнавання взагалі щось бачить) і кнопка в
  правому верхньому куті відео "Сховати/Показати індикатори на відео". Коли вимкнено —
  `drawLandmarksOverlay` отримує `null` замість `landmarksStatus` (канвас очищається і
  ховається), без жодного впливу на сам WebSocket/дані — суто UI-перемикач.

**Перевірено наживо:**
- `vitest`: 83/83 passed (+1 новий тест: кнопка реально ховає/показує canvas і перемикає
  `aria-pressed`). `tsc --noEmit`, `eslint`, `next build` — чисті.

## Перший РЕАЛЬНИЙ класифікатор: дактильна абетка (USL_alphabet_train)

Користувач завантажив у репозиторій `USL_alphabet_train/` — 37 462 реальні фото рук
(не синтетика, не demo). Це перший коли-небудь реальний навчальний матеріал у проєкті.

**Чистка (обов'язкова, до будь-якого тренування):**
- Назви папок були пошкоджені (зайвий символ рамки `│`/`╨` перед кожною кириличною
  літерою — артефакт розпакування на Windows) — перейменовано на чисті одиночні літери.
- Папки **Ъ, Ы, Э** — підтверджено з користувачем (не припущено!), що це НЕ українська
  абетка (в укр. алфавіті цих літер немає) — перенесено в
  `USL_alphabet_train/_excluded_non_ukrainian/`, нічого не видалено.
- Папка `TР` (змішаний Latin+Cyrillic, 29 фото) і папка з 1 файлом-сміттям — мітка
  неоднозначна, перенесено в `USL_alphabet_train/_excluded_unclear/` до з'ясування.
- Верхній і нижній регістр не перетинались (13 великих + 10 малих літер — по суті два
  різні часткові джерела, а не дублікати) — об'єднано в один канонічний набір великих
  літер. **Українська абетка має 33 літери, у цьому датасеті реально представлено 26**
  (відсутні: Є, Ґ, І, Ї, К, Л, Н) — чесно відсутні, не вигадані.

**`ml/fingerspelling/` (NEW) — реальний пайплайн, не demo:**
- `dataset.py` — сканує чисті папки, реальний MediaPipe `LandmarkExtractor` на кожному
  фото (перевикористовує `ml/preprocessing/normalization.py`'s `normalize_hand()`), ліву
  руку дзеркалить по X (щоб ліва й права рука того самого жесту потрапляли в один
  простір ознак) — задокументоване інженерне рішення, не вигадка. Фото без жодної руки
  або з двома руками одразу (неоднозначно для фото одного жесту) — чесно пропускається
  і логується, а не підганяється під нуль.
- `split.py` — стратифікований спліт по класах. **Це НЕ signer-independent спліт**
  (на відміну від `ml/datasets/split.py` для відео) — у цих фото взагалі немає
  інформації, хто саме жестикулює, тож цю гарантію застосувати неможливо. Чесно
  задокументовано як обмеження, а не замовчано.
- `model.py`/`train.py` — маленький MLP (63→64→64→num_classes) з вагами класів проти
  реального дисбалансу (16–3427 прикладів на клас у train).

**Реально виконано (не оцінка, справжній прогін):**
- Екстракція: **28 510 / 37 462 фото** дали рівно одну виявлену руку (76%). Пропущено
  8952: 8917 — жодної руки не знайдено (переважно клас "Ь", що впав з 1500 до **4**
  придатних фото — ці конкретні фото виявились 100×56 сірими мікро-знімками, замалими
  для детектора; клас практично непридатний, і це видно з чесного звіту, а не
  прихована), 35 — знайдено обидві руки одразу (неоднозначно).
- Перша спроба ваг класів (звичайна `1/count`) дала загальну test-точність 73%, але
  **клас "Ж" (найбільший, 3427 прикладів) впав до 1%** — за такого екстремального
  дисбалансу (16–3427, ~200×) звичайна обернена частота настільки занижує вагу
  найбільшого класу, що модель практично ігнорує його заради дрібних класів.
  Виправлено на `1/sqrt(count)` (м'якша корекція, співвідношення ваг ~14× замість
  ~200×) → **загальна test-точність зросла до 76.8%**, "Ж" піднявся до 20%.
- "Ж" лишається слабким навіть після фіксу вагів — перевірила вручну (переглянула
  реальні фото з папки): у "Ж" **перемішано щонайменше дві різні форми руки**
  (один вказівний палець піднятий vs. вказівний+середній разом) під однією міткою.
  Це забруднення мітки в самих вихідних даних, а не хиба моделі чи пайплайна —
  чесна знахідка, не прихована.
- Фінальний реальний checkpoint (`ml/fingerspelling/checkpoints/latest.pt`,
  `latest.report.json`): **test accuracy 76.8%** (n=4275), по класах:
  сильні (>90%): Г, Е, М, Р, Х, З, Т, П (усі великі, ~1500-1800 train-прикладів);
  середні (60-90%): Б, С, Ц, Ю; слабкі (<50%): Ж (мітка забруднена, вище), Д, И, Й,
  Ш, Щ; класи з <10 тестових прикладів (А, В, О, У, Ф, Я, Ь) — точність там статистично
  шумна (0% чи 100% на 4-5 прикладах нічого не доводить), і звіт це прямо показує, а не
  згладжує середнім.
- `pytest`: `ml/tests/test_fingerspelling_*.py` — 11 нових тестів (skip-логіка,
  дзеркалення руки, стратифікований спліт, forward pass, реальний тренувальний цикл на
  іграшкових даних) — усі проходять. `ruff check` — чисто.

**Known limitations (чесно, не приховано):**
- 7 літер української абетки в датасеті відсутні взагалі (Є, Ґ, І, Ї, К, Л, Н) —
  класифікатор охоплює 26/33.
- Немає signer_id — спліт не гарантує, що модель не запам'ятала чиюсь конкретну руку.
- Клас "Ж" потребує ручного передивлення й повторної розмітки (мінімум двох підтипів)
  перед тим, як довіряти йому в продакшн.

## Скрипт запису відео-датасету слів + підключення реального класифікатора дактилю до live WebSocket (позапланово)

Користувач: "давай ще додатковий скрипт для збирання відео-датасету за ноутбуку кількох слів"
і одразу після цього, мід-хід: "підключи модель нлп і модель класифікатор не демо".

**Уточнення щодо "моделі НЛП":** rule-based граматична композиція (`ml/nlp/gloss_to_text.py`,
`RuleBasedTranslationService`) вже **реальна й уже підключена** до live WebSocket — це не
demo і ніколи ним не було, це звичайний детермінований код, не ML-модель. Тут нічого не
міняла — підключати не було чого.

**`scripts/record_dataset.py` (NEW)** — інтерактивний CLI (OpenCV) для запису реальних відео
на власному ноутбуці користувача: типовий список 20 слів (`ml/nlp/lexicon.py`'s 3 займенники +
7 дієслів + 6 іменників + граматичні маркери NOT/PAST — останні множать корисність КОЖНОГО вже
наявного дієслова, не лише своєї категорії). Пише кліпи+анотації в точнісінько тому форматі
(`SampleAnnotation`), який `ml/training/train.py` вже вміє тренувати — жодного нового
тренувального коду не потрібно, щойно з'явиться достатньо реальних кліпів. Camera-логіка
(потребує реальний дисплей/вебкамеру) не виконувана в цьому sandbox; чиста логіка
(`display_label`, `next_take_index`, `build_annotation`) — 9 нових тестів.

**Підключення реального класифікатора дактилю до live WebSocket** — раніше (`ml/fingerspelling/`)
це був окремий офлайн-скрипт; тепер він реально працює в live-перекладі:

- **`ml/fingerspelling/dataset.py`** — рефакторинг: витягнула `hand_to_feature_vector()` як
  окрему чисту функцію (нормалізація + дзеркалення лівої руки), яку тепер використовують і
  offline-екстракція фото (`extract_features`), і live WebSocket-хендлер — той самий код,
  тренування й інференс ніколи не розійдуться в реалізації.
- **`ml/fingerspelling/recognizer.py`** (NEW) — завантажує checkpoint, класифікує один
  63-вимірний вектор руки (без часового вікна, на відміну від LSTM-розпізнавача слів).
- **`backend/app/services/fingerspelling_service.py` + `fingerspelling_provider.py`** (NEW) —
  дзеркалить `InferenceService`/`inference_provider.py`'s патерн лінивого завантаження
  checkpoint. Відсутній checkpoint — **не помилка** (на відміну від слів): це додаткова,
  необов'язкова можливість, шлях літер просто мовчки не активується.
- **`backend/websocket/protocol.py`** — нові `letter_prediction`/`letter_confirmed` повідомлення,
  окремі від `prediction`/`final_prediction` (ті лишаються з demo-checkpoint слів). Стабілізація
  через той самий `GlossSequenceAggregator`, що й для слів — скидається щоразу, коли в кадрі
  вже не рівно одна рука, щоб можна було жестикулювати ту саму літеру двічі підряд.
- **`frontend/`** — новий тип `LetterPredictionMessage`, `useWebSocket()`'s `letterMessage`
  (те саме "липке" значення, що й `landmarksStatus`); заразом виправила реальний, хоч і дрібний
  баг у backpressure-логіці (Phase 17 задача) — `letter_prediction`/`letter_confirmed`
  помилково потрапляли під "будь-яке повідомлення не landmarks_status/connection" і зняли б
  б блокування нового кадру ЗАРАНІШЕ, ніж прийде справжнє термінальне повідомлення кадру.
  Новий компонент `Fingerspelling` (NEW) накопичує підтверджені літери в слово, підписаний
  явно "(реальна модель, не демо)" — контраст із сусідньою панеллю "Переклад", яка й далі
  показує `[DEMO]`.

**Перевірено наживо:**
- `pytest backend/`: 35/35 passed (+3 нових: реальний натренований tiny-checkpoint через
  справжній WebSocket-потік підтверджує літеру після стабільних кадрів; відсутній checkpoint —
  мовчки, без помилки; дві руки одночасно — мовчки пропущено). `pytest ml/`: 3658/3658 passed
  (+3 нових для recognizer.py). `ruff check` — чисто (окрім наперед відомого, чужого
  `websocket/manager.py`, не мого).
- `vitest`: 91/91 passed (+10 нових: 2 для backpressure-фіксу, 6 для компонента
  `Fingerspelling`, plus косвенне покриття через оновлені хук-тести). `tsc --noEmit`, `eslint`,
  `next build` — чисті.
- **Реальний E2E через справжній production-білд**: Playwright + Chromium, підмінений
  `WebSocket` шле реальну послідовність `landmarks_status` → `letter_confirmed` (літери "П",
  потім "Р") → (незалежне) `error` слів-рівня. Підтверджено програмно, читаючи DOM: сторінка
  реально показує підпис "(реальна модель, не демо)" і накопичене слово "ПР" — не просто "код
  не впав", а підтверджене відображення в реальному браузері.

**Known limitations:** датасет для запису (20 слів) ще не записаний — `scripts/record_dataset.py`
готовий прийняти реальні кліпи, коли вони з'являться; доти слово-рівневий переклад лишається
`[DEMO]`. Дактиль-класифікатор покриває 26/33 літер (див. попередню секцію) і не інтегрований у
жоден граматичний композер — підтверджені літери лише накопичуються як сирий рядок, це
дактилювання (спелінг), а не переклад слова.
